/* VIN Demo console — Pipeline + Operations views */

/* ============================ CUSTOMERS ============================ */
function Customers({ go, selected }) {
...
function CustomerDetail({ c, go, back }) {
  ...
  return (
    <div className="page scroll">
      <PageHead crumbs={[{ label: "Departments", to: "customers" }, { label: c.name }]} go={(r) => { if (r === "customers") back(); else go(r); }}
        overline={c.seg} title={<span className="flex items-center gap-3"><Avatar initials={c.name.split(" ").map((w) => w[0]).slice(0, 2).join("")} color={c.color} size={32} />{c.name}</span>}
        actions={<><button className="btn btn-secondary"><Icon name="plus" size={13} /> Add stakeholder</button>{c.hot ? <a className="btn btn-primary" href="VIN Demo — Desktop.html"><Icon name="external" size={13} /> Open control room</a> : <button className="btn btn-primary"><Icon name="sessions" size={13} /> Plan session</button>}</>} />
      ...
      {live && (
        <div className="card" style={{ marginBottom: 22, overflow: "hidden" }}>
          <div className="card-hd" style={{ background: "var(--color-navy)", borderColor: "transparent" }}>
            ...
            <a className="btn btn-on-dark btn-sm" href="VIN Demo — Desktop.html"><Icon name="external" size={13} /> Control room</a>
          </div>
    ...
    <div className="page scroll">
      ...
      <div className="grid cols-2 section-gap">
        <div className="card card-pad">
          ...
          {[["read", "Read / navigate", "success"], ["non-destructive", "Reversible UI", "info"], ["mutating", "Writes state", "danger"]].map(([t, l, k]) => (
    ...
    <div className="page scroll">
      ...
      <div className="card section-gap" style={{ overflow: "hidden" }}>
        ...
        <table className="tbl">
          ...
          <tbody>
            {[["#312", "Approval delegation", "✓", "✓", "0", "✓", true], ["#311", "Approval delegation + interrupt", "✓", "✓", "0", "self-healed", true], ["#310", "Delegation (stale KB)", "✓", "✓", "degraded", "n/a", true], ["#309", "Bulk approval", "✓", "missed step", "0", "recovered", false], ["#308", "Approval delegation", "✓", "✓", "0", "✓", true]].map((r, i) => (
...
/* ============================ COSTS ============================ */
function Costs({ go }) {
  const { costBreakdown, sessions, customers } = VD;
...
Object.assign(window, { Customers, CustomerDetail, Sessions, Safety, MODES, Evals, Costs, Settings });
