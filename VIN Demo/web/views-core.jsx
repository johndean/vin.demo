/* VIN Demo console — core views: Dashboard, Products, Knowledge */

function PageHead({ overline, title, desc, actions, crumbs, go }) {
  return (
          ...
          <div className="breadcrumb">
            {crumbs.map((c, i) => (
              <React.Fragment key={i}>
                ...
                {c.to ? <a onClick={() => go(c.to)}>{c.label}</a> : <span style={{ color: "var(--color-navy)", fontWeight: 700 }}>{c.label}</span>}
              </React.Fragment>
            ))}
          </div>
        )}
...
function Dashboard({ go }) {
  const { sessions, products, customers } = VD;
    ...
    <div className="page scroll">
      ...
      {live && (
        <div className="card hatch" style={{ background: "var(--color-navy)", border: "none", marginBottom: 20, padding: "18px 22px", display: "flex", alignItems: "center", gap: 18, position: "relative", overflow: "hidden" }}>
          ...
          <a className="btn btn-on-dark" href="VIN Demo — Desktop.html" style={{ background: "#fff", color: "var(--color-navy)" }}><Icon name="external" size={13} /> Open control room</a>
        </div>
      )}
...
function ProductDetail({ p, go, back }) {
  ...
  return (
    <div className="page scroll">
      <PageHead crumbs={[{ label: "Products", to: "products" }, { label: p.name }]} go={(r) => { if (r === "products") back(); else go(r); }}
        ...
        actions={<><button className="btn btn-secondary"><Icon name="edit" size={13} /> Edit</button><a className="btn btn-primary" href="VIN Demo — Desktop.html"><Icon name="play" size={13} /> Launch demo</a></>} />
      <div className="tabs">{PROD_TABS.map((t) => <button key={t} className={tab === t ? "active" : ""} onClick={() => setTab(t)}>{t}</button>)}</div>
      {tab === "Overview" && <ProductOverview p={p} go={go} setTab={setTab} />}
...
function ProductOverview({ p, setTab }) {
  return (
    <>
      ...
      <div className="grid cols-2">
        <div className="card">
          ...
          <div className="card-pad" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[["Version", `v${p.version} · ${p.versions.length} tracked`, "Versions"], ["Knowledge base", `${p.chunks.toLocaleString()} chunks · trust metadata`, "Knowledge base"], ["Demo graph", `${p.graphNodes} screens · ${p.graphFlows} workflows`, "Demo graph"], ["Environment", `${p.env} · ${p.envStatus}`, "Environment"]].map(([k, v, to]) => (
        ...
        <div className="card">
          ...
          <div className="card-pad">
            ...
            {[["Validated", p.kbValidated, "var(--color-green)"], ["Needs review", p.kbReview, "var(--color-amber)"], ["Stale", p.kbStale, "var(--color-red)"]].map(([k, v, c]) => (
...
Object.assign(window, { PageHead, Dashboard, Products, ProductCard, ProductDetail });
