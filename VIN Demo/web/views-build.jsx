    ...
    <Wrap className={embedded ? "" : "page scroll"}>
      ...
      <div className="flex between items-center" style={{ marginBottom: 14, flexWrap: "wrap", gap: 12 }}>
        <div className="flex gap-2" style={{ flexWrap: "wrap" }}>
          {[["all", "All", knowledge.length], ["validated", "Validated", knowledge.filter((k) => k.status === "validated").length], ["needs-review", "Needs review", knowledge.filter((k) => k.status === "needs-review").length], ["stale", "Stale", knowledge.filter((k) => k.status === "stale").length]].map(([id, lbl, n]) => (
...
function DemoGraphInner({ p }) {
  ...
  const kindColor = { screen: "var(--color-steel)", feature: "var(--color-navy)", workflow: "var(--color-blue)" };
  return (
    <>
...
function EnvironmentInner({ p }) { return <div style={{ maxWidth: 560 }}><EnvCard p={p} /></div>; }

/* ============================ PERSONAS ============================ */
function Personas({ go }) {
  return (
    <div className="page scroll">
...
Object.assign(window, { Knowledge, DemoGraphs, DemoGraphInner, Environments, EnvironmentInner, Personas, MiniGraph });
