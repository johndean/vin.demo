/* VIN Demo — Control Room runtime. Plays the demo-loop beats.
   Hooks (useState/useEffect/useRef) come from shell.jsx (loaded first). */
    ...
    <div className="cr-col cr-right">
      <div className="cr-sec">
        ...
        <div className="flex between items-center" style={{ gap: 10 }}>
          <span className={`cr-mode ${mode}`}><Icon name={MODE_META[mode].icon} size={12} /> {MODE_META[mode].label}</span>
...
function RightPanel({ beat, mode, open, setOpen, tab, setTab, messages, typing }) {
  const TABS = [
...
function Convo({ messages, typing }) {
...
function Transport({ beat, idx, playing, onPlay, onStep, onBack, onRestart, speed, onSpeed }) {
  ...
  return (
    <div className="cr">
      <div className="cr-strip">
        ...
        <a className="cr-icon-btn" href="VIN Demo — Web.html" title="Back to console"><Icon name="external" size={16} /></a>
      </div>
...
ReactDOM.createRoot(document.getElementById("root")).render(<ControlRoom />);
