...
- [x] **Increment 3 — multi-turn loop (live).** LangGraph checkpointer (cross-turn state) + router conditional edges (`interpret → explain | resume | retrieve→navigate`). **Mid-flight pivot**: a new question pushes a context breadcrumb; "take me back" pops and returns. **explain** node answers "why did you show that?" from the decision trace. **Per-demo cost events** (Gap J) on every LLM/embedding/nav call, tagged to a `demo_session`. **Tracing** opt-in via LangSmith env (`.env.example`); decision `trace` always-on. New: `cost.ts`, `session.ts`, `convo.ts` (multi-turn demo), `eval-phase1.ts`.
  - **`npm run eval:phase1` → 8/8 PASS** — asserts the MVP definition of done (§10) live: cited answer · self-heal · never-fires-mutating · "why?" · pivot-and-return · off-topic gates · cost recorded.
  - **Adversarial review applied (3 findings folded, 3 tracked → P2).** Folded: **[1]** the `customers` get-or-create hit *no matching unique constraint* — 0002 indexed 5 spine tables but missed `customers`, so `ON CONFLICT (workspace_id, name)` raised `42P10` every run and fell into an **error-swallowing** catch (masking real DB failures); added migration `0003` (defensive de-dupe + the missing index) and made the upsert atomic. **[2]** per-turn **state bleed** — `interpret` now also resets `navigation`/`blockedMutations`/`retrieved`, so an explain/resume/gated turn can't reprint the prior turn's screen or blocked actions (the breadcrumb `currentPosition`/`contextStack` still persists by design). **[3]** `explainWhy` now guards refusal / empty content like `interpret`, instead of narrating a confident-sounding `(unable to explain)`. **Tracked, deferred to keep the repeatable 8/8 stable:** navigate-on-failure breadcrumb edge (don't move context to an unreached URL); eval coupling to self-heal *firing* + the LLM's screen pick (→ fuller P2 harness); unbounded cross-turn `trace` growth (explain *needs* cross-turn trace, so bound it in P2 rather than reset).

## MVP definition of done — **met** (8/8, repeatable via `npm run eval:phase1`).
