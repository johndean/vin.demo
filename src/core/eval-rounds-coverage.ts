/**
 * rounds.vin coverage eval — proves Knowledge + Demo Graph cover every documented page (zero gaps, 100%)
 * via the shared coverage asserter. Read-only. Run AFTER seed: npm run eval:rounds-coverage
 */
...
console.log('═══════════════════════════════════════════════════\n');
await recordEvalRun('rounds-coverage', checks.length - failed.length, checks.length, { failed: failed.map((c) => c.name), pages: PAGES.length, workflows: WORKFLOWS.length });
