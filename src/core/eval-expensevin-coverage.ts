/**
 * expense.vin coverage eval — proves Knowledge + Demo Graph cover every documented page (zero gaps, 100%)
 * via the shared coverage asserter. Read-only. Run AFTER seed: npm run eval:expensevin-coverage
 */
...
console.log('═══════════════════════════════════════════════════\n');
await recordEvalRun('expensevin-coverage', checks.length - failed.length, checks.length, { failed: failed.map((c) => c.name), pages: PAGES.length, workflows: WORKFLOWS.length });
