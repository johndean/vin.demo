...
export function FormShell({ title, subtitle, onClose, children, footer, width, grid }: { title: string; subtitle?: string; onClose: () => void; children: React.ReactNode; footer?: React.ReactNode; width?: number; grid?: boolean }) {
  return (
    <div className="form-shell" style={width ? { maxWidth: width } : undefined}>
      ...
      <div className={`form-shell__body${grid ? ' form-shell__body--grid' : ''}`}>{children}</div>
      {footer && <div className="form-shell__ft">{footer}</div>}
    </div>
  );
}

/* Small labeled field wrapper used by the CRUD forms. `full` spans the whole grid row (textareas, etc.). */
export function Field({ label, children, full }: { label: string; children: React.ReactNode; full?: boolean }) {
  return <div className={`fld${full ? ' fld--full' : ''}`}><label>{label}</label>{children}</div>;
}
