/**
 * Stateless, signed session token (HMAC-SHA256) — the hosted engine verifies a token the web
 * console issued (see apps/web/app/api/auth/login), using the SAME SESSION_SECRET, so no shared
 * DB lookup is needed to gate /session/stream. The token is `base64url(payload).base64url(sig)`
 * where payload = { email, role, exp }.
 *
 * Implemented with the Web Crypto API (crypto.subtle), available in Node 20 / tsx. No dependency.
 *
 * NOTE: intentionally duplicated from apps/web/lib/session-token.ts — the two services build in
 * independent contexts. Keep the two copies in sync.
 */
